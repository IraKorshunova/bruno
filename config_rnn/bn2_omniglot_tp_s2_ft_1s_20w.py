...
base_metadata_path = utils.find_model_metadata('metadata/', 'bn2_omniglot_tp_s2')
...
def build_model(x, init=False, sampling_mode=False):
    ...
    with arg_scope([nn_extra_nvp.conv2d_wn, nn_extra_nvp.dense_wn], init=init):
        ...
        if student_layer is None:
            student_layer = nn_extra_student.StudentRecurrentLayer(shape=(ndim,), corr_init=corr_init,
                                                                   nu_init=nu_init)

...
if __name__ == "__main__":
    ...
    # mu = 1.8438
    # K = np.eye(N) * 0.803368
    # # assert np.allclose(multivariate_student.pdf(np.array([0]),mu,K,1), 0.318309884)
    # # assert np.allclose(multivariate_student.pdf(np.array([1]),mu,K,1), 0.159154942)
    # #
    # # assert np.allclose(multivariate_student.pdf(np.array([0]),mu,K,2), 0.353553391)
    # # assert np.allclose(multivariate_student.pdf(np.array([1]),mu,K,2), 0.192450090)
    #
    #
    # for nu in xrange(3, 10):
    #     X = multivariate_student.sample(mu, K, nu)
    #     a = multivariate_student.pdf(X, mu, K, nu)
    #     b = multivariate_student.log_pdf(X, mu, K, nu)
    #     assert np.allclose(a, np.exp(b)), "log_pdf is not consistent with pdf"
    #
    #     x = np.arange(-10, 10, 0.05)
    #     y = [multivariate_student.pdf(np.array([X]), mu, K, nu) for X in x]
    #     plt.plot(x, y, 'r')
    # y = [scipy.stats.norm.pdf(np.array([X]), mu, K)[0, 0] for X in x]
    # plt.plot(x, y, 'b')
    #
    # for nu in [3, 10]:
    #     plt.figure()
    #     X = multivariate_student.sample(mu, K, nu, samples=1000000)
    #     n, bins, patches = plt.hist(X, 1000, range=[-10, 10], normed=1, facecolor='green', alpha=0.75)
    #     x = np.arange(-10, 10, 0.05)
    #     y = [multivariate_student.pdf(np.array([X]), mu, K, nu) for X in x]
    #     plt.plot(x, y, 'r')
    #

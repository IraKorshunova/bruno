...
def build_model(x, phase, init=False, sampling_mode=False, return_prior=False):
    global nvp_layers
    ...
    with tf.variable_scope("one_step") as scope:
        ...
        for i in range(seq_len):
            if sampling_mode:
                ...
                log_prob = latent_log_prob + log_det_jac[:, i]  # todo: put back
                log_probs.append(log_prob) # todo

                # if return_prior:
                #     latent_log_prob_prior = student_layer.get_log_likelihood_under_prior(z_vec[:, i, :],
                #                                                                          mask_dim=mask_dim)
                #     # log_prob_prior = latent_log_prob_prior + log_det_jac[:, i] #todo: put back
                #     prior_log_probs.append(latent_log_prob_prior) # todo
    ...
    if return_prior:
        prior_log_probs = tf.stack(prior_log_probs, axis=1)
        return log_probs, prior_log_probs
    else:
        return log_probs
